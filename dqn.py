...
def optimize(dqn, target_dqn, memory, optimizer):
    ...
    #print(action)
    action = torch.cat([action.unsqueeze(0) for action in action]).to(device)
    next_obs = torch.cat([next_obs if next_obs != None else torch.zeros([1, 4, 84, 84]).to(device) for next_obs in next_obs]).to(device)
    #print(next_obs.shape)
    reward = torch.cat([reward.unsqueeze(0) for reward in reward]).to(device)
